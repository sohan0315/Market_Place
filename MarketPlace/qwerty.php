...
if(isset($_SESSION["shopping_cart"])){
    foreach ($_SESSION["shopping_cart"] as $product){
        $order_id = rand();
        $sql = 'INSERT INTO market_place_orders (OrderID, product_name, ProductCode, OrderDate, OrderTotal, User_Name) VALUES ("'.$order_id.'","'.$product["name"].'","'.$product["product_code"].'",CURDATE(),"'.$total_price_final_temp.'","'.$_SESSION["loggedinuser"].'")';
    }
}
...
<body>
  ...
  <h4 class="text-center">Order Details - </h4></br>
  </br>';?>
...
<div class="row">
    ...
    if(isset($_SESSION["shopping_cart"])){
      ...
      foreach ($_SESSION["shopping_cart"] as $product){
          ...
          if(stripos($product["product_code"], 's')  !== false ){
              if($count == 0){
                  echo '<h6 class="card-header text-center" style="font-weight:bold; color:#17a2b8 ">
                  Store - PHD AUTOMOBILES
                  </h6>';
                  $count = $count + 1;
                  $total_price_copy = $total_price_copy + $product["product_price"];
              }
...
echo '</br>';
  ...
  if(isset($_SESSION["shopping_cart"])){
    ...
    foreach ($_SESSION["shopping_cart"] as $product){
        ...
        if(stripos($product["product_code"], 't')  !== false ){
            if($count == 0){
                echo '<h6 class="card-header text-center" style="font-weight:bold; color:#17a2b8 ">
...
if(isset($_SESSION["shopping_cart"])){
  ...
  foreach ($_SESSION["shopping_cart"] as $product){
      ...
      if(stripos($product["product_code"], 'c')  !== false ){
          if($count == 0){
              echo '<h6 class="card-header text-center" style="font-weight:bold; color:#17a2b8 ">
              Store - AutoCar Production
              </h6>';
              $count = $count + 1;
              $total_price_copy = $total_price_copy + $product["product_price"];
          }
